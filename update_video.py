...
# 1. Add handleRemoveVideoBg and file size check
old_upload = """  const handleVideoBgUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
...
new_upload = """  const handleVideoBgUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
...
# 2. Update Video Background Rendering
old_video_render = """          {/* Background Video & Overlay */}
...
new_video_render = """          {/* Background Video & Overlay */}
          <div className="absolute inset-0 w-full h-full z-0 bg-[#0a0a0a]">
            {/* Primary Moving Video Background */}
...
# 3. Update Video Upload UI
old_ui = """          {/* UPLOAD VIDEO LATAR BELAKANG */}
...
new_ui = """          {/* UPLOAD VIDEO LATAR BELAKANG */}
          <div className="bg-[#111] border border-[#333] p-6 rounded-2xl">
            <h3 className="text-[#3498db] font-black text-xl mb-1 uppercase tracking-widest flex items-center gap-2">
              🎬 Video Latar Belakang (.MP4)
            </h3>
